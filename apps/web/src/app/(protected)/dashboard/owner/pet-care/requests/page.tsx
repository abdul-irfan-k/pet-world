...
export default function MyPetCareRequestsPage() {
  const { data: requestsResponse, isLoading, error, refetch: loadRequests } = useFetchMyPetCareRequestsQuery({});

  if (isLoading) {
    return <div className="p-6 text-center">Loading your pet care requests...</div>;
  }

  if (error) {
    return <div className="p-6 text-center text-red-500">Error fetching pet care requests: {error.message}</div>;
  }

  const petCareRequests: PetCareRequest[] = requestsResponse?.data?.petCareRequests || [];
